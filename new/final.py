# General imports
#https://www.kaggle.com/kyakovlev/m5-three-shades-of-dark-darker-magic/notebook
import numpy as np
import pandas as pd
...
def make_lag_roll(LAG_DAY,base_test):
    ...
    for LAG_DAY in ROLS_SPLIT:
        shift_day = LAG_DAY[0]
        roll_wind = LAG_DAY[1]
        print(shift_day,roll_wind)
        col_name = 'rolling_mean_tmp_' + str(shift_day) + '_' + str(roll_wind)
        lag_df[col_name] = base_test[['id', 'd', TARGET]].groupby(['id'])[TARGET].transform(
            lambda x: x.shift(shift_day).rolling(roll_wind).mean())
        col_name2.append(col_name)
    return lag_df[col_name2]
...
if __name__ == "__main__":
    ...
    # PATHS for Features
    ORIGINAL = 'm5-forecasting-accuracy/'
