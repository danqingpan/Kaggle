...
def fun(asdf):
    #a  = np.array([0.1,0.1])
    a = np.array([0.02, 0.03, 0.06, 0.09, 0.14, 0.25, 0.41])
    return a.dot(np.array(asdf))
...
def create_dt(first_day,tr_last):
    ...
    for day in range(tr_last+1, tr_last+ 28 +1):
            dt[f"d_{day}"] = np.nan
    #dt=dt.drop(["d_1427","d_1792"],axis=1) 虽然这两天为0，只能在最后入模的数据中删除
...
def create_fea(dt):
    ...
    dt['decay'] = dt[["id", "lag_1"]].groupby("id")["lag_1"].transform(
        lambda x: x.rolling(7).apply(fun, raw=False))
...
#    
    ...
    sub.to_csv("submission05103.csv",index=False)
